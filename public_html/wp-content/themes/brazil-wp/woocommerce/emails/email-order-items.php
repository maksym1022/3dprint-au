...
<?php if( $_SESSION['priority_shipping'] ): ?>
<?php
	$tax_display = $order->tax_display_cart;

	if ( $tax_display == 'excl' ) {
		// Show shipping excluding tax
		$value = wc_price( $order->order_shipping, array('currency' => $order->get_order_currency()) );			
	} else {
		// Show shipping including tax
		$value = wc_price( $order->order_shipping + $order->order_shipping_tax, array('currency' => $order->get_order_currency()) );
	}
?>
		<tr>
			<td style="text-align:left; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;"><?php echo $order->get_shipping_method() ?></td>
			<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;">1</td>
			<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;"><?php echo $value ?></td>
		</tr>
<?php endif; ?>
