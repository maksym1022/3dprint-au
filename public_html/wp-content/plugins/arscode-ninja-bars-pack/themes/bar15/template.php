<div class="snp-fb snp-bar15 snp-pos-bottom <?php echo $POPUP_META['snp_open_effect']; ?>">
...
<script>
var snp_bar15_open = function()
{
    jQuery('.snp-pop-<?php echo $ID; ?> .snp-bar15').not('.snp-pos-static').addClass('snp-showme');
}
var snp_bar15_close = function()
{
    jQuery('.snp-pop-<?php echo $ID; ?> .snp-bar15').not('.snp-pos-static').fadeOut('fast',function(){ jQuery('.snp-pop-<?php echo $ID; ?> .snp-bar15').removeClass('snp-showme').show(); });
}
</script>
<?php
...
{
	echo '.snp-pop-'.$ID.' .snp-bar15 .snp-container { width: '.$POPUP_META['snp_width'].'px; }'."\n";
}
if (!empty($POPUP_META['snp_height']))
{
	echo '.snp-pop-'.$ID.' .snp-bar15 { min-height: '.$POPUP_META['snp_height'].'px; }'."\n";
}
if (!empty($POPUP_META['snp_bg_color1']))
{
	echo '.snp-pop-'.$ID.' .snp-bar15 { background-color: '.$POPUP_META['snp_bg_color1'].';}'."\n";
	echo '.snp-pop-'.$ID.' .snp-bar15 .snp-field input { background-color: '.$POPUP_META['snp_bg_color1'].';}'."\n";
}
if (!empty($POPUP_META['snp_header_font']['size']))
{
	echo '.snp-pop-'.$ID.' .snp-bar15 .snp-container h2 {font-size: '.$POPUP_META['snp_header_font']['size'].'px; color: '.$POPUP_META['snp_header_font']['color'].';}'."\n";
}
 if (!empty($POPUP_META['snp_header_font']['font']))
{
	echo '.snp-pop-'.$ID.' .snp-bar15 .snp-container h2 {font-family: "'.$POPUP_META['snp_header_font']['font'].'";}'."\n";
}
echo '</style>';
