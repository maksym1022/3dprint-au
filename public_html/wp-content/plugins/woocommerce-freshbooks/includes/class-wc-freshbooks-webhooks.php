 ...
 *
 * @package   WC-FreshBooks/Webhooks
 * @author    SkyVerge
...
class WC_FreshBooks_Webhooks {
	...
	/**
	 * Dispatch actions for the received webhook after parsing & verification
	 * is complete
	 *
	 * @since 3.0
	 */
	...
	private function dispatch_complete( $error = '' ) {
		...
		if ( $error ) {
			...
			wc_freshbooks()->log( sprintf( 'Webhook processing failed: %s, data: %s', $error, print_r( $_POST, true ) ) );
		}
