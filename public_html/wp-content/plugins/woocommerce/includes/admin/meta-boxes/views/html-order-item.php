...
<tr class="item <?php echo apply_filters( 'woocommerce_admin_html_order_item_class', ( ! empty( $class ) ? $class : '' ), $item ); ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	<td class="thumb">
		<?php if ( $_product ) : ?>
			...
			?>"><?php 
				if ( $metadata = $order->has_meta( $item_key ) ) {
					foreach ( $metadata as $meta ) {
						if($meta['meta_key'] == 'filename'){
							$filename = $meta['meta_value'];
						}
					}
				}
				$img_path = ABSPATH ."STL/". $order->billing_email ."/".date( 'd-m-Y', strtotime( $order->post->post_date_gmt ) )."/".$order->id."/".$filename.".png";
				
				//$img_path = str_replace(array(".stl",".STL"), '.png', $img_path);
				if(file_exists($img_path) && is_file($img_path)){
					$img_dir = "/STL/". $order->billing_email ."/".date_i18n( 'd-m-Y', strtotime( $order->post->post_date_gmt ) )."/".$order->id."/".$filename.".png";
					$img = '<img width="90" height="90" src="'.$img_dir.'" class="attachment-shop_thumbnail wp-post-image" alt="img-thumb">';
				}else{
					$img = wc_placeholder_img( 'shop_thumbnail' );
				}
			?>
			<?php echo $img;//$_product->get_image( 'shop_thumbnail', array( 'title' => '' ) ); ?></a>
		<?php else : ?>
